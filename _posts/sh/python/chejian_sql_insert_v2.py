...
# 源数据目录
root_folder = "/home/ubuntu/Desktop/db"
...
# 将文件中某些行输出到新文件中
def move_line_to_new_file(str, file, new_file):
    # sed "${del_start_num},${del_end_num}d" "${file_name}".copy > "${file_name}".sql
    cmd =  'sed  -n -e "' + str + '" ' + file +" > " + new_file 
    print(cmd)
    run_coomand(cmd)
...
def check_insert_str(arr=testarr):
    i=0
    tmp=''
    for item in arr:
        tmp =tmp + str(item)+"p;"
...
# sed -n -e '1,10p' -e '50,60p' $file > ${file}_new
...
def deal_one_sql_file(file, new_file):
    del_line_arr = search_file_str_num(file, "INSERT INTO")
    print(del_line_arr)
    del_line_str = check_insert_str(del_line_arr)
    print(del_line_str)
    move_line_to_new_file(del_line_str, file, new_file)
    pass
...
for file in file_lists:
    ...
    deal_one_sql_file(file, new_file)
